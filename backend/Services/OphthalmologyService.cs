    ...
    public class OphthalmologyService
    {
            ...
            ExtractSymptomsViaLlmAsync(string text, List<string> symptomsList, CancellationToken cancellationToken)
        {
            var systemPrompt =
                ...
                "}\n" +
                "Используйте только симптомы из списка. Только валидный JSON, без markdown, размышлений и пояснений.\n" +
                "ВАЖНО: Пишите ответ сразу, без внутренних рассуждений и сомнений. Не используйте теги <think>, не анализируйте свои шаги повторно, не пытайтесь перепроверить или переписать ответ в процессе генерации. Сделайте выбор один раз и сразу выведите JSON.";
